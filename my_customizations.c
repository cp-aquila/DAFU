...
static void delay_cycles(uint32_t cy)
{
  while (cy > 1) {
    cy--;
  }
...
static void i2c_master_wait_for_sync()
{
  while (SERCOM1->I2CM.SYNCBUSY.reg & SERCOM_I2CM_SYNCBUSY_MASK) {
  }
}

static void mcp_data(uint8_t reg, uint8_t data)
{
  // Set action to ACK
...
void led_toggle(void) {
  ...
  mcp_data(MCP23008_REG_GPIO, mcp23008_gpio_val);
}
...
void board_setup_late(void)
{
  ...
  // start glck4 with 8Mhz
  gclk_enable(4, GCLK_SOURCE_OSC8M, 1);
  ...
  // attach sercom1 clock to generator 4 (8Mhz)
  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID(GCLK_SERCOM1_CORE) | GCLK_CLKCTRL_CLKEN | GCLK_CLKCTRL_GEN(4);
  ...
  mcp_data(MCP23008_REG_IODIR, 0x00);
}

void board_reset_cleanup(void)
{
  // disable clocks
  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID(GCLK_SERCOMx_SLOW) | GCLK_CLKCTRL_GEN(1);
  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID(GCLK_SERCOM1_CORE) | GCLK_CLKCTRL_GEN(4);
